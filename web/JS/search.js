...
let searchQuery = params.get('key');
let searchedProducts = products.filter(product => 
  ...
  $(() => {
    ...
    function setupPagination() {
      ...
      if (searchedProducts.length === 0 || totalPages <= 1) {
        return;
      }
    ...
    function showPage(page) {
      ...
      searchedProducts.slice(start, end).forEach((product, index) => {
        $('.productCard').append(`
    ...
    function defaultSort() {
      // 在這裡實現預設排序邏輯
      searchedProducts = products.filter(product => product.category == tabValue);
      showPage(1);
    }
